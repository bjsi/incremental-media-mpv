...
function ImportSubmenu:_init()
    ...
    self.yt_chain = {
        ...
        function(args, chain, idx) self:query_split_chapters(args, chain, idx) end,
        function(args, chain, idx) self:query_priority_yt(args, chain, idx) end,
        function(args, chain, idx) self:query_confirm(args, chain, idx) end,
    }
end
...
function ImportSubmenu:query_download(args, chain, i)
    ...
    get_user_input(handler, {
            text = "Localize YouTube video? (y/[n]): ",
            replace = true,
        })
end
...
function ImportSubmenu:query_confirm(args, chain, i)
    local handle = function(input)
        ...
        local topics = importer.create_yt_topics(
            args["infos"],
            args["split-chapters"],
            args["download"],
            args["priority-min"],
            args["priority-max"],
            true -- TODO: dependencyImport
        )
...
function ImportSubmenu:import_directory(folder)
...
function ImportSubmenu:query_priority_range(args, chain, i)
    ...
    local handle = function(input)
        ...
        if not ext.validate_priority(min) or not ext.validate_priority(max) or min > max then
