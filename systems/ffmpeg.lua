...
function ffmpeg.extract_gif(url, start, stop, outputPath)
    ...
    local args = {
        ...
        "-reconnect_delay_max", "5", "-ss", tostring(start), "-t",
        tostring(stop - start), "-i", url, "-vf",
...
function ffmpeg.screenshot(url, start, outputPath)
    local args = {
        "ffmpeg", "-reconnect", "1", "-reconnect_streamed", "1",
        "-reconnect_delay_max", "5", "-ss", tostring(start), "-i", url,
        "-vframes", "1", "-q:v", "3", outputPath
    }
...
function ffmpeg.audio_extract(start, stop, audioUrl, outputPath)
    local args = {
        "ffmpeg", "-reconnect", "1", "-reconnect_streamed", "1",
        "-reconnect_delay_max", "5", "-nostats", "-ss", tostring(start), "-to",
        tostring(stop), "-i", audioUrl, "-acodec", "copy", outputPath
    }
