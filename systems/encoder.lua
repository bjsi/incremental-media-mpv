...
local function encode(args, path)
    ...
    sys.background_process(args,
                           function(status, stdout, error_string, killed_by_us)
        if status and status ~= 0 then
            log.err(string.format("encoding failed: '%s'", path))
            log.debug("exit code: " .. status .. "; stdout: " .. stdout)
        end

        log.debug(string.format("encoded '%s' in %f s", path,
                                mp.get_time() - start_time))
    end)
end
...
function encoder.any_audio(params)
    local args = {
        "mpv", params.src_path, "--o=" .. params.tgt_path,
        "--no-ocopy-metadata", "--vid=no", "--aid=" .. (params.track or "1"),
        "--sid=no", "--of=" .. params.format, "--oac=" .. params.codec,
...
function encoder.image(path, time)
    ...
    local args = {
        "mpv", helper.current_path_abs(), "--o=" .. path, "--no-ocopy-metadata",
        "--vid=" .. mp.get_property("vid"), "--aid=no", "--sid=no",
        "--start=" .. time, "--frames=1", "--of=image2", "--ovc=" .. cfg.codec,
    ...
    elseif cfg.codec == "libwebp" then
        table.insert(args, "--ovcopts-add=lossless=" ..
                         (cfg.webp.lossless and 1 or 0))
        table.insert(args,
                     "--ovcopts-add=compression_level=" .. cfg.webp.compression)
        table.insert(args, "--ovcopts-add=quality=" .. cfg.webp.quality)
    elseif cfg.codec == "png" then
        table.insert(args,
                     "--ovcopts-add=compression_level=" .. cfg.png.compression)
    end
