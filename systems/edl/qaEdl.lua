...
function QAEDL:format_silence()
    ...
    return table.concat({"silence.mp3", 0, soundLength, "title=no"}, ",")
end
...
function QAEDL:format_sound(sound)
    ...
    return table.concat({
        fname, sound["start"], -- TODO - parentStart ?
        tostring(soundLength), "showat=" .. sound["showat"]
    }, ",")
end
...
function QAEDL:write(sound, format, media)
    ...
    if media then handle:write(self:format_media(media) .. "\n") end
