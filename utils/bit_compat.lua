...
function bit_conv.to_bits(num, len)
    ...
    for i = #bits + 1, len do table.insert(bits, false) end
    return bits
end
...
function bit_conv.to_num(bits)
    ...
    local num = 0
    for i = #bits, 2, -1 do num = (num + (bits[i] and 1 or 0)) * 2 end
...
function bit_compat.extract(n, field, width)
    ...
    for i = 1, width do res[i] = bits[field + i] end
    return bit_conv.to_num(res)
end
...
function bit_compat.replace(n, v, field, width)
    ...
    for i = 1, width do res[field + i] = v_bits[i] end
    return bit_conv.to_num(res)
end
