...
function scroll_list:newline() self.ass.data = self.ass.data .. '\\N' end
...
function scroll_list:update_ass()
    ...
    if start > 1 then
        self:append(self.wrapper_style .. (start - 1) .. ' item(s) above\\N\\N')
    end

    for i = start, finish do self:format_line(i, self.list[i]) end

    if overflow then
        self:append('\\N' .. self.wrapper_style .. #self.list - finish ..
                        ' item(s) remaining')
    end
    self.ass:update()
end
...
function scroll_list:add_keybinds()
    for _, v in ipairs(self.keybinds) do
        mp.add_forced_key_binding(v[1],
                                  'dynamic/' .. self.ass.id .. '/' .. v[2],
                                  v[3], v[4])
    end
end
...
function scroll_list:ipairs() return ipairs(self.list) end

-- append item to the end of the list
function scroll_list:insert(item) self.list[#self.list + 1] = item end

local metatable = {
    __index = function(t, key)
        if scroll_list[key] ~= nil then
            return scroll_list[key]
        elseif key == "__current" then
            return t.list[t.selected]
...
function scroll_list:new()
    ...
    vars = {
        ...
        keybinds = {
            {
                'DOWN', 'scroll_down', function()
                    vars:scroll_down()
                end, {repeatable = true}
            },
            {
                'UP', 'scroll_up', function() vars:scroll_up() end,
                {repeatable = true}
